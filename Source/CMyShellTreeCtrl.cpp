...
IMPLEMENT_DYNAMIC(CMyShellTreeCtrl, CTreeCtrl)

// IContextMenu2* CMyShellTreeCtrl::m_pContextMenu2 = NULL;
...
BEGIN_MESSAGE_MAP(CMyShellTreeCtrl, CTreeCtrl)
	//{{AFX_MSG_MAP(CMyShellTreeCtrl)
	ON_WM_CREATE()
	ON_WM_CONTEXTMENU()
	ON_WM_RBUTTONDOWN()
	ON_WM_DESTROY()
	ON_NOTIFY_REFLECT(TVN_ITEMEXPANDING, &CMyShellTreeCtrl::OnItemexpanding)
	ON_NOTIFY_REFLECT(TVN_DELETEITEM, &CMyShellTreeCtrl::OnDeleteitem)
	ON_MESSAGE(WM_MFC_INITCTRL, &CMyShellTreeCtrl::OnInitControl)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CMyShellTreeCtrl message handlers

int CMyShellTreeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CMyShellTreeCtrl::SetRelatedList(CMFCShellListCtrl* pShellList)
{
	ASSERT_VALID(this);
...
CMFCShellListCtrl* CMyShellTreeCtrl::GetRelatedList() const
{
	ASSERT_VALID(this);
...
void CMyShellTreeCtrl::FillRootItems(const CString& strStartupFilePath)
{
    GetRootItems(strStartupFilePath);
}

BOOL CMyShellTreeCtrl::GetRootItems(const CString& strStartupFilePath)
{
	ASSERT_VALID(this);
	ENSURE(theApp.GetShellManager() != NULL);
	ASSERT_VALID(theApp.GetShellManager());
    ...
    LPITEMIDLIST pidl = NULL;// ::SHSimpleIDListFromPath(strStartupFilePath);

    theApp.GetShellManager()->ItemFromPath(strStartupFilePath, pidl);
    ...
    else
    {
        ...
        hr = pDesktop->BindToObject(pidl, NULL, IID_IShellFolder, (LPVOID*) &pParentFolder);
        // hr = pDesktop->GetUIObjectOf(NULL, pidl, NULL, IID_IShellFolder, (LPVOID*) &pParentFolder);
	...
	pItem->pidlRel = pidl;
	pItem->pidlFQ = theApp.GetShellManager()->CopyItem(pidl);
...
BOOL CMyShellTreeCtrl::GetChildItems(HTREEITEM hParentItem)
{
	ASSERT_VALID(this);
...
HRESULT CMyShellTreeCtrl::EnumObjects(HTREEITEM hParentItem, LPSHELLFOLDER pParentFolder, LPITEMIDLIST pidlParent)
{
	ASSERT_VALID(this);
	ASSERT_VALID(theApp.GetShellManager());
...
int CALLBACK CMyShellTreeCtrl::CompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
...
void CMyShellTreeCtrl::OnShowContextMenu(CPoint point)
{
...
CString CMyShellTreeCtrl::OnGetItemText(LPAFX_SHELLITEMINFO pItem)
{
	ENSURE(pItem != NULL);
...
int CMyShellTreeCtrl::OnGetItemIcon(LPAFX_SHELLITEMINFO pItem, BOOL bSelected)
{
	ENSURE(pItem != NULL);
...
void CMyShellTreeCtrl::OnItemexpanding(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CMyShellTreeCtrl::OnDeleteitem(NMHDR* pNMHDR, LRESULT* pResult)
{
	ASSERT_VALID(theApp.GetShellManager());
	...
	theApp.GetShellManager()->FreeItem(pItem->pidlRel);
...
void CMyShellTreeCtrl::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
	if (m_bContextMenu)
	{
...
BOOL CMyShellTreeCtrl::SelectPath(LPCTSTR lpszPath)
{
	ASSERT_VALID(this);
	ASSERT_VALID(theApp.GetShellManager());
	...
	LPITEMIDLIST pidl;
	if (FAILED(theApp.GetShellManager()->ItemFromPath(lpszPath, pidl)))
	{
		return FALSE;
	}
...
BOOL CMyShellTreeCtrl::SelectPath(LPCITEMIDLIST lpidl)
{
	BOOL bRes = FALSE;
	...
	if (theApp.GetShellManager()->GetItemCount(lpidl) == 0)
	{
		// Desktop
	}
	else
	{
		...
		lstItems.AddHead(theApp.GetShellManager()->CopyItem(lpidl));

		while (theApp.GetShellManager()->GetParentItem(lpidlCurr, lpidlParent) > 0)
		{
		...
		for (POSITION pos = lstItems.GetHeadPosition(); pos != NULL;)
		{
			...
			theApp.GetShellManager()->FreeItem(lpidlList);
		}
	}
...
BOOL CMyShellTreeCtrl::GetItemPath(CString& strPath, HTREEITEM htreeItem) const
{
	ASSERT_VALID(this);
...
void CMyShellTreeCtrl::OnRButtonDown(UINT /*nFlags*/, CPoint point)
{
	SetFocus();
	UINT nFlags = 0;
...
void CMyShellTreeCtrl::EnableShellContextMenu(BOOL bEnable)
{
	m_bContextMenu = bEnable;
}

BOOL CMyShellTreeCtrl::OnChildNotify(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pLResult)
{
...
void CMyShellTreeCtrl::OnDestroy()
{
...
LRESULT CMyShellTreeCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
...
void CMyShellTreeCtrl::PreSubclassWindow()
{
...
void CMyShellTreeCtrl::InitTree()
{
...
void CMyShellTreeCtrl::SetFlags(DWORD dwFlags, BOOL bRefresh)
{
	ASSERT_VALID(this);
	m_dwFlags = dwFlags;
...
LRESULT CMyShellTreeCtrl::OnInitControl(WPARAM wParam, LPARAM lParam)
{
